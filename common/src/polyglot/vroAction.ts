...
export class VroAction {
    ...
    static async fromRemoteState(client: VroRestClient, action: VroAction): Promise<VroAction | null> {
        ...
        const remoteActionTags = remoteCatalogContents.attributes
            .find((a: any) => a.name === "globalTags")
            .value?.split(" ")
        const reamoteActionDescription = remoteCatalogContents.attributes
            .find((a: any) => a.name === "description")
            .value?.split(" ")
