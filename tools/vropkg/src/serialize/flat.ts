...
const serializeFlatDunesMetadata = async (context: any, pkg: t.VroPackageMetadata): Promise<void> => {
    ...
    node.ele("comment").text("UTF-16").up()
        ...
        .ele("entry").att("key", "pkg-name").text(getPackageName(pkg)).up()
        .ele("entry").att("key", "pkg-description").text("Built by vRealize Build Tools").up()
...
const serializeFlatSignatures = async (context: any, pkg: t.VroPackageMetadata): Promise<void[]> => {
    ...
    glob.sync(target + "/**/*", { nodir: true }).forEach(async file => {
        const location = file.replace(target, "");
        const data = s.sign(fs.readFileSync(file), pkg.certificate);
        promises.push(context.signatures(location)(data));
    })
    return promises;
}
